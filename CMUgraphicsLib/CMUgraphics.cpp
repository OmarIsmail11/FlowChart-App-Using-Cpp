...
window::~window() {
    ...
    if(dcScreen != NULL) {
        if(ReleaseDC(hwndWindow, dcScreen) == 0) {
		    std::cout << "Fatal Error: Failed to release dcScreen in window class destructor!" << endl;        
        }
	...
	if (dcActive != NULL) {
		if(DeleteDC(dcActive) != TRUE) {
		    std::cout << "Fatal Error: Failed to delete dcActive in window class destructor!" << endl;
		}
	}
	
	if(penObject != NULL) { 
	    if(DeleteObject(penObject) != TRUE) {
	        std::cout << "Fatal Error: Failed to delete penObject in window class destructor!" << endl;	        	    
	    }
		penObject = NULL; 
	}
	...
	if(brshObject != NULL) { 
	    if(DeleteObject(brshObject) != TRUE) {
	        std::cout << "Fatal Error: Failed to delete brshObject in window class destructor!" << endl;	        	    	    
	    }
		brshObject = NULL; 
	}
	...
	if(fontObject != NULL) {
	    if(DeleteObject(fontObject) != TRUE) {
	        std::cout << "Fatal Error: Failed to delete fontObject in window class destructor!" << endl;	        	    	    
	    }
		fontObject = NULL; 
	}
    ...
    if(bmapBuffer != NULL) {
    	if(DeleteObject(bmapBuffer) != TRUE) {
	        std::cout << "Fatal Error: Failed to delete bmapBuffer in window class destructor!" << endl;	        	    
    	}
    	bmapBuffer = NULL;
    }
    ...
    if(wipInput->Count() == 1) {
        if(wipInput->bWaitClose == true) {
          FlushMouseQueue();
          // Can't use std::cout because it's destructor might be called before the 
...
void window::ChangeDrawStyle(drawstyle dsNewStyle) {
    ...
    switch(dsNewStyle) {
      ...
      default:
        std::cout << "Fatal Error: You have tried to use an invalid or unimplemented drawstyle!" << endl;
		break;
  	}
...
bool window::SetBuffering(const bool bSetting) {
	...
	if(bDoubleBuffer == true) {
	    ...
	    SelectObject(dcActive, bmapBufferOld);
  	    ...
  	    if(dcActive != NULL) {
		    if(DeleteDC(dcActive) != TRUE) {
		        std::cout << "Fatal Error: Failed to delete dcActive in SetBuffering!" << endl;
		    }
		    dcActive = NULL;
	    }
	    ...
	    if(bmapBuffer != NULL) {
	        if(DeleteObject(bmapBuffer) != TRUE) {
	            std::cout << "Fatal Error: Failed to delete bmapBuffer in SetBuffering!" << endl;	    	            
	        }
	        bmapBuffer = NULL;
	    }
...
void window::UpdateBuffer() {
	...
	if(bDoubleBuffer != false) {
        if(BitBlt(dcScreen, 0, 0, iWindowWidth - 1, iWindowHeight - 1, dcActive, 0, 0, SRCCOPY) != TRUE) {
            std::cout << "Fatal Error: Failed to BitBlt in UpdateBuffer!" << endl;
        }
	}
}
...
color window::SetBrush(const unsigned char ucRed, const unsigned char ucGreen, const unsigned char ucBlue) {
	...
	if (brshObject != NULL) { 
	    if(DeleteObject(brshObject) != TRUE) {
	        std::cout << "Fatal Error: Failed to delete brshObject in SetBrush!" << endl;	    	    
	    }
		brshObject = NULL; 
	}
...
color window::SetPen(const unsigned char ucRed, const unsigned char ucGreen, const unsigned char ucBlue, const int iWidth) {
	...
	if (penObject != NULL) { 
	    if(DeleteObject(penObject) != TRUE) {
	        std::cout << "Fatal Error: Failed to delete penObject in SetPen!" << endl;	    
	    }
		penObject = NULL; 
	}
...
void window::SetFont(const int iSize, const unsigned short usStyle, const fontfamily ffFamily, const char *cFontName) {
	...
	if(fontObject != NULL) { 
	    if(DeleteObject(fontObject) != TRUE) {
            std::cout << "Fatal Error: Failed to delete fontObject in SetFont!" << endl;	        	    		    	    
	    }
		fontObject = NULL; 
	}
	...
	switch(dsStyle) {
	  case FRAME:
	    ...
	    if(MoveToEx(dcActive, iX1, iY1, NULL) != TRUE) {
	        std::cout << "Fatal Error: Failed to MoveToEx in DrawLine!" << endl;	        	    		    
	    }
	    if(LineTo(dcActive, iX2, iY2) != TRUE) {
            std::cout << "Fatal Error: Failed to draw in DrawLine!" << endl;	        	    		    
	    }
	    RestoreDrawStyle();
		break;
	...
	if((iWidth == 0) && (iHeight == 0)) {
	    ...
	    switch(dsStyle) {
	        ...
	        if(Rectangle(dcActive, iX1, iY1, iX2, iY2) != TRUE) {
                std::cout << "Fatal Error: Failed to draw filled style in DrawRectangle!" << endl;	        	    		    
	        }
	        RestoreDrawStyle();
	        break;
		  ...
		  case FRAME:
            ChangeDrawStyle(FRAME);
 	        if(Rectangle(dcActive, iX1, iY1, iX2, iY2) != TRUE) {
                std::cout << "Fatal Error: Failed to draw frame style in DrawRectangle!" << endl;	        	    		    
 	        }
 	        RestoreDrawStyle();
			break;
		  ...
		  case INVERTED:
			...
			if(Rectangle(dcActive, iX1, iY1, iX2, iY2) != TRUE) {
                std::cout << "Fatal Error: Failed to draw inverted style in DrawRectangle!" << endl;	        	    		    
			}
			RestoreDrawStyle();
			break;
	    ...
	    switch(dsStyle) {
	        ...
	        if(RoundRect(dcActive, iX1, iY1, iX2, iY2, iWidth, iHeight) != TRUE) {
                std::cout << "Fatal Error: Failed to draw filled style in DrawRectangle (rounded)!" << endl;	        	    		    
	        }
	        RestoreDrawStyle();
			break;
		  ...
		  case FRAME:
			ChangeDrawStyle(FRAME);
			if(RoundRect(dcActive, iX1, iY1, iX2, iY2, iWidth, iHeight) != TRUE) {
                std::cout << "Fatal Error: Failed to draw frame style in DrawRectangle (rounded)!" << endl;	        	    		    
			}
			RestoreDrawStyle();
			break;
		  ...
		  case INVERTED:
			...
			if(InvertRgn(dcActive,rgnTemp) != TRUE) {
                std::cout << "Fatal Error: Failed to draw inverted style in DrawRectangle (rounded)!" << endl;	        	    		    			
			}
...
void window::DrawRhombus(const int iX1, const int iY1, const int iX2, const int iY2, const int iX3, const int iY3, const int iX4, const int iY4, const drawstyle dsStyle)
{
	...
	case FILLED:
		...
		if (Polygon(dcActive, ptVertices, 4) != TRUE) {
			std::cout << "Fatal Error: Failed to draw filled style in DrawTriangle!" << endl;
		}
		RestoreDrawStyle();
		break;
	...
	case FRAME:
		ChangeDrawStyle(FRAME);
		if (Polygon(dcActive, ptVertices, 4) != TRUE) {
			std::cout << "Fatal Error: Failed to draw frame style in DrawTriangle!" << endl;
		}
		RestoreDrawStyle();
		break;
	...
	case INVERTED:
		...
		if (InvertRgn(dcActive, rgnTemp) != TRUE) {
			std::cout << "Fatal Error: Failed to draw inverted style in DrawTriangle!" << endl;
		}
		DeleteObject(rgnTemp);
		break;
...
void window::DrawArrow2(const int iX1, const int iY1, const int iX2, const int iY2, const drawstyle dsStyle) {

	DrawLine(iX1, iY1, iX2, iY2);
	float Nslope = 0;
	if ((iY1 - iY2))
		 Nslope = -float((iX2 - iX1)) / float((iY2 - iY1));

	
	int x1 = iX2 + 5;
	int y1 = Nslope * (x1 - iX2) + iY2;



	int x2 = iX2 - 5;
	int y2 = (Nslope * (x2 - iX2)) + iY2;
	
	int x3 = iX2 + 10;
	int y3 = abs((1 / Nslope)) * (x3 - iX2) + iY2;
...
void window::DrawArrow(const int iX1, const int iY1, const int iX2, const int iY2, const char out, const char in, const drawstyle dsStyle)
{
	...
	switch (in) {
	case 'U':
		DrawLine(iX2, iY2, iX2, iY2 - 5);
		DrawTriangle(iX2 + 5, iY2 -5, iX2 - 5, iY2 -5, iX2, iY2, FILLED);
		break;
	case 'D':
		DrawLine(iX2, iY2, iX2, iY2 - 5);
		DrawTriangle(iX2 + 5, iY2 - 5, iX2 - 5, iY2 - 5, iX2, iY2 - 10, FILLED);
		break;
	case 'R':
		
		DrawTriangle(iX2, iY2 + 5, iX2, iY2 - 5, iX2 + 5, iY2, FILLED);
		break;
	case 'L':
		DrawLine(iX2, iY2, iX2 + 5, iY2);
		DrawTriangle(iX2+5, iY2 + 5, iX2+5, iY2 - 5, iX2, iY2, FILLED);
		break;
	}
...
void window::DrawPolygon(const int* ipX, const int* ipY, const int iVertices, const drawstyle dsStyle) {
    ...
    switch(dsStyle) {
      case FILLED:
		ChangeDrawStyle(FILLED);	    
 	    if(Polygon(dcActive, ptVertices, iVertices) != TRUE) {
            std::cout << "Fatal Error: Failed to draw filled style in DrawPolygon!" << endl;	        	    		    
 	    }
 	    RestoreDrawStyle();
		break;
	  ...
	  case FRAME:
		ChangeDrawStyle(FRAME);
 	    if(Polygon(dcActive, ptVertices, iVertices) != TRUE) {
            std::cout << "Fatal Error: Failed to draw frame style in DrawPolygon!" << endl;	        	    		     	    
 	    }
 	    RestoreDrawStyle();
		break;
	  ...
	  case INVERTED:
	    ...
	    if(InvertRgn(dcActive, rgnTemp) != TRUE) {
            std::cout << "Fatal Error: Failed to draw inverted style in DrawPolygon!" << endl;	        	    		    
	    }
		DeleteObject(rgnTemp);
		break;
...
void window::DrawCircle(const int iX, const int iY, const int iRadius, const drawstyle dsStyle) {
    ...
    switch(dsStyle) {
      case FILLED:
		ChangeDrawStyle(FILLED);	    
 	    if(Ellipse(dcActive, (iX - iRadius), (iY - iRadius), (iX + iRadius), (iY + iRadius)) != TRUE) {
            std::cout << "Fatal Error: Failed to draw filled style in DrawCircle!" << endl;	        	    		     	    
 	    }
 	    RestoreDrawStyle();
		break;
	  ...
	  case FRAME:
		ChangeDrawStyle(FRAME);
 	    if(Ellipse(dcActive, (iX - iRadius), (iY - iRadius), (iX + iRadius), (iY + iRadius)) != TRUE) {
 	        std::cout << "Fatal Error: Failed to draw frame style in DrawCircle!" << endl;	        	    		    
	    }
 	    RestoreDrawStyle();
		break;
	  ...
	  case INVERTED:
		rgnTemp = CreateEllipticRgn((iX - iRadius), (iY - iRadius), (iX + iRadius), (iY + iRadius));
 	    if(InvertRgn(dcActive, rgnTemp) != TRUE) {
            std::cout << "Fatal Error: Failed to draw inverted style in DrawCircle!" << endl;	        	    		     	    
 	    }
		DeleteObject(rgnTemp);
		break;
	...
	switch(dsStyle) {
      case FILLED:
		...
		if(Ellipse(dcActive, iX1, iY1, iX2, iY2) != TRUE) {
            std::cout << "Fatal Error: Failed to draw filled style in DrawEllipse!" << endl;	        	    		    
		}
		RestoreDrawStyle();
		break;
	  ...
	  case FRAME:
		ChangeDrawStyle(FRAME);
 	    if(Ellipse(dcActive, iX1, iY1, iX2, iY2) != TRUE) {
            std::cout << "Fatal Error: Failed to draw frame style in DrawEllipse!" << endl;	        	    		     	    
 	    }
 	    RestoreDrawStyle();
		break;
	  ...
	  case INVERTED:
	    rgnTemp = CreateEllipticRgn(iX1, iY1, iX2, iY2);
 	    if(InvertRgn(dcActive, rgnTemp) != TRUE) {
            std::cout << "Fatal Error: Failed to draw inverted style in DrawEllipse!" << endl;	        	    		     	    
 	    }
		DeleteObject(rgnTemp);
		break;
	...
	switch(dsStyle) {
	  case FILLED:
	    ...
	    if(Pie(dcActive, iX1, iY1, iX2, iY2, iStartX, iStartY, iEndX, iEndY) != TRUE) {
            std::cout << "Fatal Error: Failed to draw filled style in DrawArc!" << endl;	        	    		    
	    }
	    RestoreDrawStyle();
		break;
	  ...
	  case FRAME:
	    ...
	    if(Arc(dcActive, iX1, iY1, iX2, iY2, iStartX, iStartY, iEndX, iEndY) != TRUE) {
            std::cout << "Fatal Error: Failed to draw frame style in DrawArc!" << endl;	        	    		    
	    }
	    RestoreDrawStyle();
		break;
	  ...
	  case INVERTED:
        ChangeDrawStyle(INVERTED);
	    if(Pie(dcActive, iX1, iY1, iX2, iY2, iStartX, iStartY, iEndX, iEndY) != TRUE) {
            std::cout << "Fatal Error: Failed to draw inverted style in DrawArc!" << endl;	        	    		    
	    }
	    RestoreDrawStyle();
		break;
	...
	switch(dsStyle) {
	  case FRAME:
	    ...
	    if(PolyBezier(dcActive, ptControlPts, 4) != TRUE) {
            std::cout << "Fatal Error: Failed to draw in DrawBezier!" << endl;	        	    		  
	    }
	    RestoreDrawStyle();
		break;
...
void window::DrawString(const int iX, const int iY, const char *cpText) {
	...
	if(SetBkMode(dcActive, TRANSPARENT) == 0) {
        std::cout << "Fatal Error: Failed to SetBkMode in DrawString!" << endl;	        	    	
	}
	if(SetTextAlign(dcActive, TA_UPDATECP) == GDI_ERROR) {
        std::cout << "Fatal Error: Failed to SetTextAlign in DrawString!" << endl;   
	}
	
	if(MoveToEx(dcActive, iX, iY, NULL) != TRUE) {
        std::cout << "Fatal Error: Failed to MoveToEx in DrawString!" << endl;	        	    	
    }

	if(TextOut(dcActive, iX, iY, cpText, (int) strlen(cpText)) == 0) {
        std::cout << "Fatal Error: Failed to TextOut in DrawString!" << endl;	        	    	
	}
	RestoreFontStyle();
...
void window::GetStringSize(int &iWidth, int &iHeight, const char* cpText) {
    ...
    if(GetTextExtentPoint32(dcActive, cpText, (int) strlen(cpText), &szTemp) != TRUE) {
        std::cout << "Fatal Error: Failed to GetTextExtentPoint32 in GetStringSize!" << endl;	        	    
    }
    RestoreFontStyle();
	  ...
	  if((imgThis->itType == JPEG) || (imgThis->itType == SCREEN)) {
		...
		if((iWidth != -1) && (iHeight != -1)) {
			if(StretchDIBits(dcActive, iX, iY, iWidth, iHeight, 0, 0, imgThis->usWidth, imgThis->usHeight, imgThis->ucpImageData, imgThis->bmiImage, DIB_RGB_COLORS, SRCCOPY) == GDI_ERROR) {
 	            std::cout << "Fatal Error: Failed to StretchDIBits in DrawImage!" << endl;
			}
		} else {
	        if(SetDIBitsToDevice(dcActive, iX, iY, imgThis->usWidth, imgThis->usHeight, 0, 0, 0, imgThis->usHeight, imgThis->ucpImageData, imgThis->bmiImage, DIB_RGB_COLORS) == 0) {
 	            std::cout << "Fatal Error: Failed to SetDIBitsToDevice in DrawImage!" << endl;
	        }
		}

	  } else {
	
        std::cout << "Fatal Error: Unsupported image type!" << endl;
...
color window::GetColor(const int iX, const int iY) {
	...
	if(BitBlt(dcTempBuffer, 0, 0, 1, 1, dcActive, iX, iY, SRCCOPY) != TRUE) {
	    std::cout << "Fatal Error: Failed to BltBit in GetColor!" << endl;
    }

	if(GetDIBits(dcActive, bmapTempBuffer, 0, 1, caImageData, bmiImage, DIB_RGB_COLORS) == 0) {
        std::cout << "Fatal Error: Failed to GetDIBits in GetColor!" << endl;	    
	}
    ...
    if(DeleteObject(bmapTempBuffer) != TRUE) {
	    std::cout << "Fatal Error: Failed to delete bmapTempBuffer in GetColor!" << endl;
    }
	if(DeleteDC(dcTempBuffer) != TRUE) {
	    std::cout << "Fatal Error: Failed to delete dcTempBuffer in GetColor!" << endl;
	}
	delete [] bmiImage;
...
void window::StoreImage(image *imgThis, const unsigned usX, const unsigned short usY, const unsigned short usWidth, const unsigned short usHeight) {
    ...
    if((usX + usWidth > iWindowWidth) || (usY + usHeight > iWindowHeight)) {
        std::cout << "Fatal Error: StoreImage selection extends outside the window!" << endl;
        return;
    }
	...
	if(BitBlt(imgThis->dcTempBuffer, 0, 0, (int) usWidth, (int) usHeight, dcActive, (int) usX, (int) usY, SRCCOPY) != TRUE) {
        std::cout << "Fatal Error: Failed to BitBlt in StoreImage!" << endl;	
	}

	if(GetDIBits(dcActive, imgThis->bmapTempBuffer, 0, usHeight, imgThis->ucpImageData, imgThis->bmiImage, DIB_RGB_COLORS) == 0) {
        std::cout << "Fatal Error: Failed to GetDIBits in StoreImage!" << endl;
	}

    SelectObject(imgThis->dcTempBuffer, bmapOld);
    if(DeleteObject(imgThis->bmapTempBuffer) != TRUE) {
	    std::cout << "Fatal Error: Failed to delete imgThis->bmapTempBuffer in StoreImage!" << endl;    
    }
	if(DeleteDC(imgThis->dcTempBuffer) != TRUE) {
	    std::cout << "Fatal Error: Failed to delete imgThis->dcTempBuffer in StoreImage!" << endl;
	}
...
void window::Print() {
    ...
    if(StartDoc(dcPrinter, &diStuff) <= 0) {
        std::cout << "Fatal Error: Failed to StartDoc in Print!" << endl;
    }    
    
    if(StartPage(dcPrinter) <= 0) {
        std::cout << "Fatal Error: Failed to StartPage in Print!" << endl;
    }  
    ...
    if((GetDeviceCaps(dcPrinter, RASTERCAPS) & RC_STRETCHBLT) != RC_STRETCHBLT) {
        ...
        if(BitBlt(dcPrinter, iXPos, iYPos, iWindowWidth, iWindowHeight, dcActive, 0, 0, SRCCOPY) != TRUE) {
            std::cout << "Fatal Error: Failed to BitBlt in Print!" << endl;   
        }
        ...
        if(StretchBlt(dcPrinter, 0, iYPos, iPrinterWidth, iPrinterHeight, dcActive, 0, 0, iWindowWidth, iWindowHeight, SRCCOPY) != TRUE) {
            std::cout << "Fatal Error: Failed to StretchBlt in Print!" << endl;
        }
        
    }
    
    if(EndPage(dcPrinter) <= 0) {
        std::cout << "Fatal Error: Failed to EndPage in Print!" << endl;
    }    
    
    if(EndDoc(dcPrinter) <= 0) {
        std::cout << "Fatal Error: Failed to EndDoc in Print!" << endl;
    }    
    
  	if(DeleteDC(dcPrinter) != TRUE) {
  	    std::cout << "Fatal Error: Failed to delete dcPrinter in Print!" << endl;
  	}
}
